...
class LSTMLumpedWeighted(nn.Module):
    ...
    def __init__(
        ...
        input_size = (1 if use_rain else 0) + (2 if use_cal else 0)
        self.lstm = nn.LSTM(
            input_size=input_size,
            hidden_size=hidden_size,
            num_layers=num_layers,
...
class LSTMDistributed(nn.Module):
    ...
    def __init__(
        ...
        # Input: P por ottobacia + hora + mes (+ PET media, se use_pet)
        input_size = n_otto + 2 + (1 if use_pet else 0)
...
class LSTMWithTTDSCS(nn.Module):
    ...
    def _montar_x(self, precip, Pe, Q_routed, hour, month):
        q_routed_feat = torch.log1p(Q_routed).unsqueeze(-1)
        hour_feat = hour.unsqueeze(-1)
        month_feat = month.unsqueeze(-1)
        if self.pe_only:
            pe_feat = torch.log1p(Pe)
            return torch.cat([pe_feat, q_routed_feat, hour_feat, month_feat], dim=-1)
        precip_feat = torch.log1p(precip)
        pe_mean_feat = torch.log1p(Pe.mean(dim=-1, keepdim=True))
        return torch.cat([precip_feat, pe_mean_feat, q_routed_feat, hour_feat, month_feat], dim=-1)
...
class LSTMWithTTDTopmodel(nn.Module):
    ...
    def _montar_x(self, precip, Pe, Q_routed, hour, month, Q_bf=None, S_bar=None):
        q_routed_feat = torch.log1p(Q_routed).unsqueeze(-1)
        hour_feat = hour.unsqueeze(-1)
        month_feat = month.unsqueeze(-1)
        if self.pe_only:
    ...
    def forward(
        ...
        if self.continuous:
            ...
            x = self._montar_x(precip, Pe, Q_routed, hour, month, Q_bf=Q_bf, S_bar=sbar_series)
            lstm_out, (hn, cn) = self.lstm(x, st.get("lstm"))
            pred_log = self.decoder(lstm_out).squeeze(-1)
            pred = torch.expm1(F.relu(pred_log))
            return {'pred': pred_log, 'pred_exp': pred, 'Pe': Pe, 'Q_routed': Q_routed,
